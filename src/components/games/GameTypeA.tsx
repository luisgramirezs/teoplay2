...
/** Tarjeta arrastrable — @dnd-kit maneja mouse y touch vía PointerSensor/TouchSensor */
...
const DraggableCard: React.FC<DraggableCardProps> = ({ realIdx, cardState, fontSize }) => {
...
/** Zona de destino — isOver de useDroppable reemplaza el antiguo estado manual dropHover */
interface DropZoneProps {
    zona: 1 | 2;
    cat: JuegoA['categoria1'];
    zonCards: CardState[];
    isDown: boolean;
    fontSize: string;
}

const DropZone: React.FC<DropZoneProps> = ({ zona, cat, zonCards, isDown, fontSize }) => {
    const { setNodeRef, isOver } = useDroppable({ id: `zona-${zona}` });

    return (
        <div
            ref={setNodeRef}
            className={`drop-zone border-2 rounded-2xl p-4 min-h-[160px] transition-all ${isOver
...
const GameTypeA: React.FC<GameTypeAProps> = ({
    ...
    const activeRealIdx = activeId ? Number(activeId.replace('card-', '')) : null;
    const activeCard = activeRealIdx !== null ? cards[activeRealIdx] : null;

    const handleDrop = (zona: 1 | 2, realIdx: number) => {
        attemptsRef.current++;
        ...
        if (isCorrect) {
            ...
            onCorrect();
            setCards(prev =>
                prev.map((c, i) => i === realIdx ? { ...c, placed: zona, feedback: 'correct' } : c)
            );
        } else {
            errorsRef.current++;
            onWrong();
            ...
            setCards(prev =>
                prev.map((c, i) => i === realIdx ? { ...c, feedback: 'wrong' } : c)
            );
            ...
            setTimeout(() => {
                setCards(prev =>
                    prev.map((c, i) => i === realIdx ? { ...c, feedback: null } : c)
                );
            }, 600);
        }
    };
    ...
    const handleDragEnd = (event: DragEndEvent) => {
        ...
        const realIdx = active.data.current?.realIdx as number | undefined;
        const zona = over.id === 'zona-1' ? 1 : over.id === 'zona-2' ? 2 : null;
        if (realIdx === undefined || zona === null) return;

        handleDrop(zona, realIdx);
    };
        ...
        >
            <div className="max-w-2xl mx-auto">
                <div className="bg-card rounded-3xl border border-border shadow-lg p-6">
                    ...
                    {/* Overlay visual del ítem mientras se arrastra — imprescindible en touch, donde el dedo tapa el elemento original */}
                    <DragOverlay>
                        {activeCard ? (
                            <div className={`drag-card flex items-center gap-2 px-5 py-3 bg-card border-2 border-primary rounded-xl font-bold shadow-xl ${fontSize} scale-105`}>
                                <DynamicIcon name={activeCard.item.icono} className="w-6 h-6 text-primary" />
                                <span>{activeCard.item.texto}</span>
                            </div>
                        ) : null}
                    </DragOverlay>
