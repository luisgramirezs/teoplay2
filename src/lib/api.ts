...
function buildPrompt(perfil: PerfilNino, escenaGramatical?: { url: string; descripcion: string } | null): string {
    ...
    const prompt = [
        ...
        `34b. Para asignatura matemáticas, física o química: si el objetivo de aprendizaje es PROCEDIMENTAL, DEBES generar "apoyoOperativo" con 1-2 "ejemplosResueltos" (cada paso con "descripcion" y "resultadoParcial", puede ser conciso — el niño ya ve el resultado ahí, es un ejemplo resuelto) y un "ejercicio" (mismo procedimiento, valores distintos). "ejercicio.pasosGuia" es DISTINTO y más importante: es lo que el niño lee dentro de un modal de ayuda mientras intenta resolver el ejercicio SOLO en su cuaderno, sin ver ningún resultado. Cada paso de "pasosGuia" debe ser una explicación AMPLIA como un tutor paciente hablándole directo al niño: explica QUÉ hacer, POR QUÉ se hace ese paso (qué significa, para qué sirve dentro del procedimiento completo) y CON QUÉ números de "ejercicio.enunciado" — 2 a 3 oraciones por paso, no una línea seca. NUNCA incluyas el resultado de ese paso ni de ningún cálculo — el niño calcula y decide solo. Ejemplo de la calidad esperada para "multiplica 2/3 por 1/4": "Cuando multiplicamos dos fracciones, el primer paso es multiplicar los numeradores entre sí. El numerador nos dice cuántas partes tenemos de cada fracción, así que al multiplicarlos sabemos cuántas partes juntas vamos a tener en total. En tu ejercicio, multiplica el numerador de la primera fracción por el numerador de la segunda: 2 por 1. Hazlo en tu cuaderno y luego sigue al siguiente paso." "ejercicio.opciones": al menos 3, una correcta, 2+ incorrectas con errores típicos, cada una con "explicacion" cálida. Si el objetivo es ESTRUCTURAL: "apoyoOperativo": null.`,
