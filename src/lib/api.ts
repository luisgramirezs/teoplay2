...
function buildPrompt(perfil: PerfilNino, escenaGramatical?: { url: string; descripcion: string } | null): string {
    ...
    const prompt = [
        ...
        "ejercicio.pasosGuia" es OBLIGATORIO y debe contener TODOS los pasos necesarios para enseñar al niño a resolver EL EJERCICIO PROPUESTO.

        Los pasosGuia NO son pistas ni instrucciones breves. Son una tutoría paso a paso.

        Cada elemento de "pasosGuia" representa UNA MICRO-EXPLICACIÓN que el niño verá individualmente al pulsar "Siguiente paso".

        Cada paso debe explicar de manera explícita y concreta:
