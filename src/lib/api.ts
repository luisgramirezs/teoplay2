...
function buildPrompt(perfil: PerfilNino, escenaGramatical?: { url: string; descripcion: string } | null): string {
    ...
    const prompt = [
        ...
        `34b. Para asignatura matemáticas, física o química: si el objetivo de aprendizaje es PROCEDIMENTAL (ejecutar pasos que transforman una entrada en un resultado — ej. sumar, calcular un porcentaje, despejar), DEBES generar "apoyoOperativo" con 1-2 "ejemplosResueltos" (enunciado, pasos numerados en orden con "descripcion" del qué y POR QUÉ de cada paso —nunca solo el comando seco, explica la razón como lo haría un tutor paciente— y "resultadoParcial" tras cada paso, "resultado" final) y un "ejercicio" con "enunciado" nuevo (mismo procedimiento, valores distintos a los del ejemplo). El "ejercicio" DEBE traer "pasosGuia": los mismos pasos del procedimiento pero aplicados a los números EXACTOS de "ejercicio.enunciado" (nunca a los del ejemplo), cada uno con "descripcion" explicando qué hacer y por qué en ESE paso — SIN incluir el resultado parcial ni la respuesta, el niño la calcula. Además, "opciones": al menos 3 (una correcta con el resultado real de "ejercicio.enunciado", 2+ incorrectas con errores típicos), cada una con su "explicacion". Si el objetivo es ESTRUCTURAL: devuelve "apoyoOperativo": null. El tono es de tutor calmado y paciente en TODO el bloque, nunca examen, nunca infantilizado.`,
