...
import { construirBloquePlantillaGramatical } from './grammar/grammarTemplateEngine';
...
function buildPrompt(perfil: PerfilNino, escenaGramatical?: { url: string; descripcion: string } | null): string {
    ...
    const prompt = [
        ...
        `14. apoyoVisual es OBLIGATORIO a nivel de explicacion para las asignaturas donde aplica (ver 14d/14e/14f). Nunca dentro de conceptosClave.`,
        `14b. "explicacion.conceptosClave" DEBE estar completo. Está prohibido devolver conceptosClave vacío.`,
        `14c. Para asignatura artes o sociales: apoyoVisual puede omitirse (tipo "ninguna") porque se usan imágenes reales de Wikimedia.`,
        `14d. Para asignatura ciencias: usa tipos de ciclo, flujo o nodos para procesos; y deja que Wikimedia aporte la imagen real complementaria.`,
        `14f. METODOLOGÍA PEDAGÓGICA Y ESTRUCTURA (CPA + DUA):`,
        `    Toda lección debe estructurarse obligatoriamente bajo el enfoque Concreto -> Pictórico -> Abstracto:`,
        `    - CONCRETO (Manipulable): En "materiales" y en la apertura de "pasosGuiados" / "intro", inicia SIEMPRE con una acción física utilizando objetos reales/cotidianos que el estudiante pueda tocar o manipular.`,
        `    - PICTÓRICO (Representativo): En "conceptosClave", explica el principio mediante modelos visuales, esquemas o diagramas sencillos. Si la asignatura lo requiere, activa "requiereApoyoVisual": true y especifica en "ejemploPedagogico" los valores exactos a representar (solo para conceptos que se beneficien de un modelo gráfico claro, no para notación abstracta pura).`,
        `    - ABSTRACTO (Simbólico): Introduce fórmulas, símbolos o notación matemática/gramatical SOLO al final, como la conclusión de la experiencia concreta y visual.`,
        `    - REFORZAMIENTO Y TONO: Si el objetivo es procedimental, incluye en "reforzamiento" actividades híbridas con ejemplos resueltos paso a paso (explicando el POR QUÉ) y preguntas con errores típicos en los distractores. Redacta todo como un tutor DUA: cálido, paciente, claro, empático y NUNCA infantilizado.`,
        ...
        `28c. Para CUALQUIER pieza cuyo rol admita genuinamente varias formas válidas en el idioma enseñado (ej. palabra interrogativa: what/where/when; verbo principal: varios verbos reales; complemento: varias frases reales) — incluye entre 2 y 4 valores distintos, nunca uno solo, para que el niño pueda practicar con varias combinaciones reales. EXCEPCIÓN: si la pieza es genuinamente invariante en este contexto específico (ej. un auxiliar fijo como "had" en pasado perfecto, o una negación fija como "not"), un solo valor es correcto y NO debe expandirse artificialmente — la cantidad de valores debe reflejar la realidad gramatical real de esa pieza en esa lección puntual, nunca un número fijo impuesto de antemano.`,
        ...
        `29b. "piezas[].valores[].correspondeA" es OBLIGATORIO en TODOS los valores de una pieza cuyos valores varíen según el sujeto (conjugación verbal, concordancia, alomorfos) — esto incluye SIEMPRE auxiliares con variación (ej. "have"/"has", "do"/"does") y el verbo to be ("am"/"is"/"are"), entre otros casos equivalentes. Ej.: pieza "verbo to be" con valores "am"/"is"/"are": cada uno debe llevar "correspondeA" con el sujeto exacto en el idioma enseñado ("I", "he / she / it", "you / we / they"), usando el mismo texto literal que la pieza de sujeto/pronombre equivalente, sin traducir de nuevo. Si varios valores comparten la misma condición (ej. "walks", "eats", "runs" con "he / she / it"), repite el mismo "correspondeA" en cada uno — el frontend los agrupa automáticamente. Si la pieza tiene valores libres sin condición (complemento, objeto), omite "correspondeA" o déjalo vacío en TODOS los valores de esa pieza — nunca parcialmente. NUNCA omitas "correspondeA" en una pieza que sí varía por sujeto — aunque el patrón te parezca obvio, el frontend depende de este campo para agrupar y explicar la concordancia.`,
        ...
        '1. ACCIÓN DIRECTA PARA EL NIÑO: Usa verbos en imperativo dirigido al estudiante (Tú). Cero lenguaje de profesor.',
        '2. FORMATO DE FRACCIONES: Escribe las fracciones directamente como \\frac{a}{b} sin paréntesis ni símbolos raros alrededor.',
        'Ejemplo de objeto CPA:',
        '"cpa": {',
        '  "concreto": "Toma un papel, dóblalo en 4 partes y colorea \\frac{2}{4}. ¡Luego colorea \\frac{1}{4} más!",',
        '  "pictorico": "Dibuja un círculo dividido en 4 partes e imagina que te comes \\frac{3}{4} de la pizza.",',
        '  "abstracto": "Escribe en tu cuaderno la suma: \\frac{2}{4} + \\frac{1}{4} = \\frac{3}{4}"',
        '}',
        '',
